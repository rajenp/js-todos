...
(function() {
    ...
    var TODOService = {
        _loader: myLib.AjaxLoader,
        API_URL: "http://localhost:9898/todo/tasks",
        ...
        clearTask: function(taskId, callback) {
            var endPoint = this.API_URL + "/" + taskId + "/delete";
            this._loader.request({
                method: "DELETE",
                url: endPoint,
                callback: callback
        ...
        search: function(props, callback) {
            var endPoint = this.API_URL + "/search",
                query = "";
            props = props || {};
            myLib.$each(props, function(value, key) {
                query += key + "=" + value + "&";
            });
            this._loader.request({
                method: "GET",
    ...
    myLib.Impls(TODOService, IService);

    /**
     * Local TODO storage. TODOs are cleared on page refresh.
     */

    var LocalTODOService = {
        _storage:(function() {
            var storage = {
                    tasks: {}
                },
        ...
        getTasks: function(callback) {
            var tasks = [];
            myLib.$each(this._storage.get("tasks"), function(task) {
                tasks.push(task);
            });
        ...
        search: function(props, callback) {
            ...
            myLib.$each(alltasks, function(task) {
                var emptyProps = true,
                    match = false;
                myLib.$each(props, function(value, prop) {
                    emptyProps = false;
    ...
    myLib.Impls(LocalTODOService, IService);
    
    window.LocalTODOService = LocalTODOService;
    window.RemoteTODOService = TODOService;
